...
forest_plot <- create_forest_plot(model1_results,
                                  "Linear Regression for the T2D Cohort", 
                                  "Beta Estimate (95% CI)", "lin")

ggsave("Plots/linear_raw_cross_T2D.png",
       forest_plot,
       dpi = 300)


#Writing the results in an exel sheet
wb <- createWorkbook("Results/Results_Cross_T2D.xlsx")
addWorksheet(wb,
...
subset_df1 <- clinical_df_T2D %>%
  select(eid,
         ...
         diabetes_duration_baseline,
         diabetes_duration_followup)
...
metabolites_eGFR <- metabolites_eGFR %>% 
  ...
  mutate(cholesterol_lowering_medication_baseline = ifelse(followup == 1,
                                                           cholesterol_lowering_medication_followup, 
                                                           cholesterol_lowering_medication_baseline)) %>%
  mutate(diabetes_duration_baseline = ifelse(followup == 1,
                                             diabetes_duration_followup,
                                             diabetes_duration_baseline)) %>%
...
forest_plot <- create_forest_plot(model2_results,
                                  "Adjusted Linear Regression for the T2D Cohort",
                                  "Beta Estimate (95% CI)",
                                  "lin")

ggsave("Plots/linear_adjusted_cross_T2D.png",
       forest_plot,
       dpi = 300)
addWorksheet(wb,
...
# Create forest plot
forest_plot <- create_forest_plot(model3_results,"Adjusted Logistic Regression for the T2D Cohort",
                                  "Odds Ratio (95% CI)",
                                  "log")

ggsave("Plots/logistic_adjusted_cross_T2D.png",
       forest_plot, 
       dpi = 300)
...
saveWorkbook(wb, 
             "Results/Results_Cross_T2D.xlsx",
             overwrite = TRUE)

####Baseline Characteristics for T2D#####
# Cases
cases_T2D <- nrow(metabolites_CKD)

# Cases with CKD
cases_with_CKD_T2D <- sum(metabolites_CKD$CKD_baseline == 1)
cases_with_cholesterol_T2D <- (sum(metabolites_CKD$cholesterol_lowering_medication_baseline == 1) / cases_T2D) * 100

# Cases Male
males_percentage_T2D <- (sum(metabolites_CKD$sex_f31_0_0 == 1) / cases_T2D) * 100

# Smoking
smoking_table_T2D <- table(metabolites_CKD$current_tobacco_smoking_f1239_0_0)
smoking_T2D <- paste(names(smoking_table_T2D), ":", round((smoking_table_T2D/cases_T2D)*100,1))

# Age
age_mean_T2D <- mean(metabolites_CKD$age_when_attended_assessment_centre_f21003_0_0)
age_sd_T2D <- sd(metabolites_CKD$age_when_attended_assessment_centre_f21003_0_0)

# BMI
BMI_mean_T2D <- mean(metabolites_CKD$body_mass_index_bmi_f21001_0_0)
BMI_sd_T2D <- sd(metabolites_CKD$body_mass_index_bmi_f21001_0_0)

# SBP
SBP_mean_T2D <- mean(metabolites_CKD$systolic_blood_pressure_automated_reading_f4080_0_0)
SBP_sd_T2D <- sd(metabolites_CKD$systolic_blood_pressure_automated_reading_f4080_0_0)

# Diaetes Duration
dd_mean_T2D <- mean(metabolites_CKD$diabetes_duration_baseline)
dd_sd_T2D <- sd(metabolites_CKD$diabetes_duration_baseline)

# hba2c
hba1c_mean_T2D <- mean(metabolites_CKD$glycated_haemoglobin_hba1c_f30750_0_0)
hba1c_sd_T2D <- sd(metabolites_CKD$glycated_haemoglobin_hba1c_f30750_0_0)

# Create the baseline characteristics table
baseline_table_T2D <- data.frame(
  Variable = c("n",
               "No. CKD Cases",
               "Males (%)",
               "Age Mean (SD)",
  ...
  T2D = c(cases_T2D,
          cases_with_CKD_T2D,
          males_percentage_T2D,
          paste(round(age_mean_T2D, 1), "(", round(age_sd_T2D, 1), ")"),
          paste(round(BMI_mean_T2D, 1), "(", round(BMI_sd_T2D, 1), ")"),
          paste(round(SBP_mean_T2D, 1), "(", round(SBP_sd_T2D, 1), ")"),
          paste(round(hba1c_mean_T2D, 1), "(", round(hba1c_sd_T2D, 1), ")"),
          paste(smoking_T2D, collapse = ", "),
          cases_with_cholesterol_T2D,
          paste(round(dd_mean_T2D, 1), "(", round(dd_sd_T2D, 1), ")"))
)
