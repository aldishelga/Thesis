...
if (length(significant_metabolites_cox) > 0){
  ...
  ggsave("Plots/Long/coxRawT2D.png",
...
if (length(significant_metabolites_linear) > 0){
  ...
  ggsave("Plots/Long/linearRawT2D.png",
         create_forest_plot(linear_raw,
                            "Raw Linear Regression for T2D Cohort",
                            "Beta Estimate (95% CI)",
                            "lin"),
         dpi = 300)
...
if (nrow(cox_adjusted) > 0){
  ...
  ggsave("Plots/Long/coxAdjustedT2D.png",
...
if (nrow(linear_adjusted) > 0){
  ...
  ggsave("Plots/Long/linearAdjustedT2D.png",
         create_forest_plot(linear_adjusted,
                            "Adjusted Linear Regression for T2D Cohort",
                            "Beta Estimate (95% CI)",
                            "lin"),
         dpi = 300)
...
wb <- createWorkbook("Results/Results_Long_T2D.xlsx")
...
saveWorkbook(wb, "Results/Results_Long_T2D.xlsx", overwrite = TRUE)

####Characteristics####
cases_T2D <- nrow(df_cox)

# Compute the number of cases with NDM
cases_with_CKD_T2D <- sum(df_cox$CKD_baseline == 1)
cases_with_cholesterol_T2D <- (sum(df_cox$cholesterol_lowering_medication_baseline == 1) / cases_T2D) * 100

# Compute the percentage of males
males_percentage_T2D <- (sum(df_cox$sex_f31_0_0 == 1) / cases_T2D) * 100

# Compute the table of smoking categories
smoking_table_T2D <- table(df_cox$current_tobacco_smoking_f1239_0_0)
...
age_sd_T2D <- sd(df_cox$age_when_attended_assessment_centre_f21003_0_0)

BMI_mean_T2D <- mean(df_cox$body_mass_index_bmi_f21001_0_0)
BMI_sd_T2D <- sd(df_cox$body_mass_index_bmi_f21001_0_0)

SBP_mean_T2D <- mean(df_cox$systolic_blood_pressure_automated_reading_f4080_0_0)
SBP_sd_T2D <- sd(df_cox$systolic_blood_pressure_automated_reading_f4080_0_0)

dd_mean_T2D <- mean(df_cox$diabetes_duration_baseline)
dd_sd_T2D <- sd(df_cox$diabetes_duration_baseline)

hba1c_mean_T2D <- mean(df_cox$glycated_haemoglobin_hba1c_f30750_0_0)
hba1c_sd_T2D <- sd(df_cox$glycated_haemoglobin_hba1c_f30750_0_0)